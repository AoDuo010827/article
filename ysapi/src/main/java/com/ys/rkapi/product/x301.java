...
public class x301 extends YS {
    static final String RTC_PATH = "/sys/devices/platform/soc/ffd00000.cbus/ffd1c000.i2c/i2c-3/3-0051/rtc/rtc0/time";
    static final String[] LED_PATH = new String[]{"/sys/devices/platform/misc_power_en/led_red"};
    private static final String BACKLIGHT_IO_PATH = "/sys/devices/platform/backlight/bl_en";

    public static final String PROP_STATUSBAR_STATE_LU = "persist.sys.navbarstate";
    public static final String PROP_SWIPE_STATUSBAR_LU = "persist.sys.navbarslide";
    public static final String PROP_SWIPE_NOTIFIBAR_LU = "persist.sys.disexpandbar";


    public static final x301 INSTANCE = new x301();
    private x301(){}
    @Override
    ...
    public void rotateScreen(Context context, String degree) {
        ...
        Utils.setValueToProp("persist.sys.displayrot",degree);
            ...
            Utils.setValueToProp("persist.sys.displayrot","0");
...
//        }
    ...
    public boolean getNavBarHideState(Context context) {
        return Utils.getValueFromProp(PROP_STATUSBAR_STATE_LU).equals("0");
    }

    @Override
    public boolean isSlideShowNavBarOpen() {
        return Utils.getValueFromProp(PROP_SWIPE_STATUSBAR_LU).equals("1");
    }

    @Override
    public void setSlideShowNavBar(Context context, boolean flag) {
        if (!flag)
            Utils.setValueToProp(PROP_SWIPE_STATUSBAR_LU, "0");
        else
            Utils.setValueToProp(PROP_SWIPE_STATUSBAR_LU, "1");
    }

    @Override
    public boolean isSlideShowNotificationBarOpen() {
        return Utils.getValueFromProp(PROP_SWIPE_NOTIFIBAR_LU).equals("0");
    }

    @Override
    public void setSlideShowNotificationBar(Context context, boolean flag) {
        if (!flag)
            Utils.setValueToProp(PROP_SWIPE_NOTIFIBAR_LU, "1");
        else
            Utils.setValueToProp(PROP_SWIPE_NOTIFIBAR_LU, "0");
    }
...
//            Utils.setValueToProp("persist.sys.usb.otg.mode","2");
            GPIOUtils.writeStringFileFor7(new File("/sys/class/unifykeys/name"),"customer1_key");
            GPIOUtils.writeStringFileFor7(new File("/sys/class/unifykeys/write"),"1");
        } else {
//            Utils.setValueToProp("persist.sys.usb.otg.mode","1");
            GPIOUtils.writeStringFileFor7(new File("/sys/class/unifykeys/name"),"customer1_key");
            GPIOUtils.writeStringFileFor7(new File("/sys/class/unifykeys/write"),"0");
        }
    }
